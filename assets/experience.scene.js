...
export function createScene({ canvas, onError }){
  ...
  function resize(){
    ...
    if (w < 2 || h < 2) return;
    renderer.setSize(w,h,false);          // false → keep CSS size (canvas fills stage via CSS)
    camera.aspect = w/h; camera.updateProjectionMatrix();
  }
  window.addEventListener("resize", resize);
  if (window.ResizeObserver){ try{ new ResizeObserver(resize).observe(canvas); }catch(e){} }
  [0,100,400,1000].forEach(ms => setTimeout(()=>requestAnimationFrame(resize), ms));  // backstops for late layout
